					     ...
					     struct wl12xx_vif *wlvif)
{
	if (wlvif->channel_type == NL80211_CHAN_HT40MINUS ||
		...
		if (wlvif->band != IEEE80211_BAND_5GHZ)
			wl1271_error("using 40Mhz AP in 2.4Ghz band - not officially supported");
...
module_param_array_named(rx_mask, rx_mask_param, int, &rx_mask_param_argc,
			 S_IRUSR);
MODULE_PARM_DESC(rx_mask, "Allow modifying the mcs supported rates. For example: rx_mask=0xff,0xff,0,0,0,0,0,0,0,0");
